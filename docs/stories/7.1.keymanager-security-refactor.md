...
- [ ] Add new encryption environment variables to `.env.example`
- [ ] Update `src/config.js` to load and validate new encryption configuration
- [ ] Implement secure default values and backward compatibility logic
- [ ] Add configuration validation unit tests
- [ ] Create encryption parameter derivation function with scrypt

### Task 2: Remove Hardcoded Encryption Constants (AC: 1)
- [ ] Remove hardcoded SALT, PASSWORD, ENCRYPTION_KEY from keyManager.js
- [ ] Replace with environment variable-based key derivation using scrypt
- [ ] Implement configurable encryption parameters (iterations, memory, parallelism)
- [ ] Update encryption/decryption methods to use new key derivation
- [ ] Add unit tests for new encryption logic

### Task 3: Backward Compatibility and Migration (AC: 3)
- [ ] Implement format detection for old vs new encrypted key files
- [ ] Create automatic migration logic for existing key files
- [ ] Add migration progress display and error handling
- [ ] Create key backup mechanism before migration
- [ ] Add integration tests for complete migration workflow
- [ ] Verify migrated keys work correctly with new system

### Task 4: Enhanced Security Features (AC: 4)
- [ ] Add private key entropy validation and weak key rejection
- [ ] Implement decryption attempt limiting with exponential backoff
- [ ] Add memory cleanup functions for sensitive data
- [ ] Implement key file integrity verification using HMAC
- [ ] Add security-related unit tests for all new features
- [ ] Add timing attack protection for decryption operations

### Task 5: Comprehensive Security Testing (AC: 5)
- [ ] Create security test suite covering all encryption scenarios
- [ ] Add tests for migration failure scenarios and recovery
- [ ] Implement memory leak detection tests for sensitive data
- [ ] Add performance benchmarks for encryption operations
- [ ] Create tests for concurrent access and thread safety
- [ ] Verify test coverage exceeds 90% for security-critical code

### Task 6: Documentation and User Guidance (AC: 6)
- [ ] Update `.env.example` with new encryption variables and explanations
- [ ] Add migration guide to README.md with step-by-step instructions
- [ ] Update API documentation for KeyManager class
- [ ] Create security best practices documentation
- [ ] Add troubleshooting section for common migration issues
- [ ] Document all new environment variables with usage examples

### Task 7: Integration and Regression Testing (AC: 7)
- [ ] Test all CLI commands with new KeyManager to ensure no breaking changes
- [ ] Verify key creation, storage, and retrieval workflows work correctly
- [ ] Test error handling and user-friendly error messages
- [ ] Add end-to-end tests for complete key management workflows
- [ ] Performance testing to ensure no degradation in key operations
- [ ] Verify all existing tests still pass with new implementation
...
- Story prepared following BMAD create-next-story task requirements
- Technical context gathered from existing keyManager.js implementation
- Architecture alignment verified with project coding standards and structure
- Security requirements based on industry best practices for cryptographic key management
- Migration strategy designed to preserve existing user data while upgrading security

### File List
**To be Created:**
- `src/utils/encryptionUtils.js` - Encryption utility functions
- `src/utils/securityUtils.js` - Security validation and cleanup
- `src/__tests__/keyManager.security.test.js` - Security test suite
- `src/__tests__/keyManager.migration.test.js` - Migration tests

**To be Modified:**
- `src/utils/keyManager.js` - Remove hardcoded constants, implement secure encryption
- `src/config.js` - Add encryption environment variable loading
- `.env.example` - Add new encryption variable templates
- `README.md` - Update security and migration documentation
